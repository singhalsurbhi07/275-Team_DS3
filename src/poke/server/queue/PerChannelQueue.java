							...
							if (res.getHeader().getRoutingId().equals(Routing.DOCQUERY)) {
								...
								if (res.getHeader().getReplyCode().equals(ReplyStatus.FAILURE)) {
									Header fb = Header.newBuilder().setOriginator(Server.getConf()
												.getServer().getProperty("node.id")).setTag(res.getHeader().getTag())
												.setIsExternal(true).setRemainingHopCount(3).build();

									Payload pb = Payload.newBuilder().build();

									Request newReq = Request.newBuilder().setBody(pb).setHeader(fb).build();
									enqueueRequest(newReq);

								} else {
									Header fb = Header.newBuilder(res.getHeader()).setOriginator(Server.getConf()
												.getServer().getProperty("node.id")).setToNode(res.getHeader().getOriginator())
												.setRoutingId(Routing.DOCFIND).build();
