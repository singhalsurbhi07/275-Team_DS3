...
public class HeartbeatManager extends Thread {
     ...
     * @param sa
     */
    public void addOutgoingChannel(String nodeId, String host, int mgmtport, Channel ch,
	    SocketAddress sa) {
	...
	while (forever) {
		...
		if (outgoingHB.size() > 0) {
			...
			try {
			    ...
			    logger.error(
				    "Failed to send HB to " + hd.getNodeId() + " at "
					    + hd.getHost(), e);
			}
		    }
		} else
	...
	public void operationComplete(ChannelFuture future) throws Exception {
		...
		logger.warn("HB outgoing channel closing for node '" + heart.getNodeId() + "' at "
			+ heart.getHost());
		outgoingHB.remove(future.getChannel());
	    } else if (incomingHB.containsValue(heart)) {
		logger.warn("HB incoming channel closing for node '" + heart.getNodeId() + "' at "
			+ heart.getHost());
